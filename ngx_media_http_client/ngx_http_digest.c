 ...
 */
static const void *
ngx_md5_body(ngx_md5_ctx *ctx, const void *data, unsigned long size)
{
...
static void
ngx_md5_update(ngx_md5_ctx *ctx, const void *data, unsigned long size)
{
  ...
  if (used) {
    ...
    size -= available;
    ngx_md5_body(ctx, ctx->buffer, 64);
  }

  if (size >= 64) {
    data = ngx_md5_body(ctx, data, size & ~(unsigned long)0x3f);
    size &= 0x3f;
  }
...
static void
ngx_md5_final(unsigned char *result, ngx_md5_ctx *ctx)
{
  unsigned long used, available;
 ...
 */
static void
ngx_get_md5(const char *string, char *result)
{
	int i = 0;
	unsigned char digest[16];

	ngx_md5_ctx context;
	ngx_md5_init(&context);
	ngx_md5_update(&context, string, strlen(string));
	ngx_md5_final(digest, &context);
 ...
 */
static char *
ngx_dgst_get_val(char *parameter)
{
	char *cursor, *q;
 ...
 */
int
ngx_check_string(const char *string)
{
 ...
 */
static char *
ngx_crop_sentence(const char *header_value)
{
 ...
 */
static inline int
ngx_split_string_by_comma(char *sentence, char **values, int max_values)
{
	int i = 0;
...
ngx_tokenize_sentence(char *sentence, char **values, unsigned int max_values)
{
	unsigned int i = 0;
 ...
 */
static int
ngx_parse_digest(ngx_digest_s *dig, const char *digest_string)
{
	int n, i = 0;
	char *val, *parameters;
	char *values[12];

	parameters = ngx_crop_sentence(digest_string);
	n = ngx_tokenize_sentence(parameters, values, ARRAY_LENGTH(values));
	...
	while (i < n) {
		...
		if (0 == strncmp("nonce=", val, strlen("nonce="))) {
			dig->nonce = ngx_dgst_get_val(val);
		} else if (0 == strncmp("realm=", val, strlen("realm="))) {
			dig->realm = ngx_dgst_get_val(val);
		} else if (0 == strncmp("qop=", val, strlen("qop="))) {
			char *qop_options = ngx_dgst_get_val(val);
			char *qop_values[2];
			int n_qops = ngx_split_string_by_comma(qop_options, qop_values, ARRAY_LENGTH(qop_values));
			while (n_qops-- > 0) {
			...
			dig->opaque = ngx_dgst_get_val(val);
		} else if (0 == strncmp("algorithm=", val, strlen("algorithm="))) {
			char *algorithm = ngx_dgst_get_val(val);
 ...
 */
static int
ngx_parse_validate_attributes(ngx_digest_s *dig)
{
	if (-1 == ngx_check_string(dig->username)) {
		return -1;
	}
	if (-1 == ngx_check_string(dig->password)) {
		return -1;
	}
	if (-1 == ngx_check_string(dig->uri)) {
		return -1;
	}
	if (-1 == ngx_check_string(dig->realm)) {
		return -1;
	}
	...
	/* nonce */
	if (DIGEST_QOP_NOT_SET != dig->qop && -1 == ngx_check_string(dig->nonce)) {
		return -1;
	}
 ...
 */
static void
ngx_hash_generate_a2(char *result, const char *method, const char *uri)
{
	char raw[512];
	sprintf(raw, "%s:%s", method, uri);
	ngx_get_md5(raw, result);
}
 ...
 */
static void
ngx_hash_generate_a1(char *result, const char *username, const char *realm, const char *password)
{
	char raw[768];
 ...
 */
static void
ngx_hash_generate_response_auth(char *result, const char *ha1, const char *nonce, unsigned int nc, unsigned int cnonce, const char *qop, const char *ha2)
{
	char raw[512];
 ...
 */
void
ngx_hash_generate_response(char *result, const char *ha1, const char *nonce, const char *ha2)
{
	char raw[512];
...
int
ngx_digest_init(ngx_digest_t *digest,unsigned int quotes)
{
	ngx_digest_s *dig = (ngx_digest_s *) digest;

	/* Clear */
	memset(dig, 0, sizeof (ngx_digest_s));
...
int
ngx_digest_is_digest(const char *header_value)
{
...
void *
ngx_digest_get_attr(ngx_digest_t *digest, ngx_digest_attr_t attr)
{
	ngx_digest_s *dig = (ngx_digest_s *) digest;
...
int
ngx_digest_set_attr(ngx_digest_t *digest, ngx_digest_attr_t attr, const ngx_digest_attr_value_t value)
{
	ngx_digest_s *dig = (ngx_digest_s *) digest;
...
int
ngx_digest_client_parse(ngx_digest_t *digest, const char *digest_string)
{
	ngx_digest_s *dig = (ngx_digest_s *) digest;
	...
	return ngx_parse_digest(dig, digest_string);
}
...
ngx_digest_client_generate_header(ngx_digest_t *digest, char *result, int max_length)
{
	...
	if (-1 == ngx_parse_validate_attributes(dig)) {
		return -1;
	}
	...
	/* Generate the hashes */
	ngx_hash_generate_a1(hash_a1, dig->username, dig->realm, dig->password);
	ngx_hash_generate_a2(hash_a2, method_value, dig->uri);

	if (DIGEST_QOP_NOT_SET != dig->qop) {
		ngx_hash_generate_response_auth(hash_res, hash_a1, dig->nonce, dig->nc, dig->cnonce, qop_value, hash_a2);
	} else {
		ngx_hash_generate_response(hash_res, hash_a1, dig->nonce, hash_a2);
	}
...
int
ngx_digest_server_parse(ngx_digest_t *digest, const char *digest_string)
{
	ngx_digest_s *dig = (ngx_digest_s *) digest;
...
int
ngx_digest_server_generate_nonce(ngx_digest_t *digest)
{
	//ngx_digest_s *dig = (ngx_digest_s *) digest;
 ...
 */
size_t
ngx_digest_server_generate_header(ngx_digest_t *digest, char *result, size_t max_length)
{
	ngx_digest_s *dig = (ngx_digest_s *) digest;
