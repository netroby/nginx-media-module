...
typedef ngx_digest_s ngx_digest_t;
 ...
 *
 */
int ngx_digest_init(ngx_digest_t *digest,unsigned int quotes);
 ...
 */
int ngx_digest_is_digest(const char *header_value);

/**
 * Get an attribute from a digest context.
 *
 * @param ngx_digest_t *digest The digest context to get attribute from.
 * @param ngx_digest_attr_t attr Which attribute to get.
 *
 * @returns void * The attribute value, a C string or a pointer to an int.
 */
void * ngx_digest_get_attr(ngx_digest_t *digest, ngx_digest_attr_t attr);

/**
 * Set an attribute on a digest object.
 *
 * @param ngx_digest_t *digest The digest context to set attribute to.
 * @param ngx_digest_attr_t attr Which attribute to set.
 ...
 */
int ngx_digest_set_attr(ngx_digest_t *digest, ngx_digest_attr_t attr, const ngx_digest_attr_value_t value);

/*********************************client************************************/
/**
 * Parse a digest string.
 *
 * @param ngx_digest_t *digest The digest context.
 * @param char *digest_string The header value of the WWW-Authenticate header.
 *
 * @returns int 0 on success, otherwise -1.
 */
int ngx_digest_client_parse(ngx_digest_t *digest, const char *digest_string);
 ...
 *  - Method
 *
 * @param ngx_digest_t *digest The digest context to generate the header value from.
 ...
 * @returns int 0 on success, otherwise -1.
 */
int ngx_digest_server_parse(ngx_digest_t *digest, const char *digest_string);
 ...
 * @returns int 0 on success, otherwise -1.
 */
int ngx_digest_server_generate_nonce(ngx_digest_t *digest);
 ...
 * Returns the number of bytes in the result string. -1 on failure.
 */
size_t ngx_digest_server_generate_header(ngx_digest_t *digest, char *result, size_t max_length);
