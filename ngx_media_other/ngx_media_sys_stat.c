...
ngx_int_t
ngx_media_sys_stat_add_disk(u_char* strDiskPath)
{
    ngx_uint_t                 i = 0;
    ...
    for(i = 0; i < g_VideoSysStat->DiskCount; i++)
    {
        ...
        if(0 == ngx_strncmp(diskInfo->m_strDiskName, strDiskPath, ngx_strlen(strDiskPath)))
        {
            bFindFlag = 1;
            break;
        }
    ...
    last = ngx_cpymem((u_char *)&diskInfo->m_strDiskName[0], strDiskPath, ngx_strlen(strDiskPath));
    *last = '\0';
...
ngx_media_sys_stat_get_diskinfo(u_char* strDiskPath, uint64_t* ullTotalSize, uint64_t* ullUsedSize)
{
    ...
    *ullUsedSize  = 0;
    ...
    {
        diskInfo = g_VideoSysStat->DiskInfoList[i];
        if(0 == ngx_strncmp((const char*)&diskInfo->m_strDiskName[0], strDiskPath, ngx_strlen(strDiskPath)))
        {
...
ngx_media_sys_stat_stat_diskinfo()
{
    ...
    for(i = 0; i < g_VideoSysStat->DiskCount; i++)
    {
        ...
        if((DISK_MOUNT == diskInfo->m_nMountFlag)
            && (NGX_OK != ngx_media_sys_stat_check_diskmount_state((u_char*)&diskInfo->m_strDiskName[0])))
        {
            ngx_log_error(NGX_LOG_WARN, g_VideoSysStat->log, 0,
                    "The Disk's mount flag is offline.diskPath[%s].",diskInfo->m_strDiskName);
        ...
        if (-1 == statvfs(diskInfo->m_strDiskName, &hdstat))
        {
            ngx_log_error(NGX_LOG_WARN, g_VideoSysStat->log, 0,
                    "Fail to read disk information.disk[%s].",diskInfo->m_strDiskName);
        ...
        {
            ngx_log_error(NGX_LOG_WARN, g_VideoSysStat->log, 0,
                    "Disk's filesystem is realonly.disk[%s].",diskInfo->m_strDiskName);
