...
#include <ngx_core.h>
#include <ngx_http.h>
#include "ngx_media_upload_progress_module.h"
...
} ngx_media_upload_progress_state_t;

typedef struct {
    ngx_str_t                                     name;
    ngx_media_upload_progress_state_t        idx;
} ngx_media_upload_progress_state_map_t;

typedef struct ngx_media_upload_progress_node_s ngx_media_upload_progress_node_t;

struct ngx_media_upload_progress_node_s {
    ...
    ngx_uint_t                                    done;
    time_t                                        timeout;
    struct ngx_media_upload_progress_node_s *prev;
    struct ngx_media_upload_progress_node_s *next;
    u_char                                        len;
    u_char                                        data[1];
};
...
typedef struct {
    ...
    time_t                                        timeout;
} ngx_media_upload_progress_cleanup_t;

typedef struct {
    ngx_rbtree_t                                 *rbtree;
    ngx_media_upload_progress_node_t         list_head;
    ngx_media_upload_progress_node_t         list_tail;
} ngx_media_upload_progress_ctx_t;

typedef struct {
    ngx_array_t                                  *values;
    ngx_array_t                                  *lengths;
} ngx_media_upload_progress_template_t;
...
} ngx_media_upload_progress_conf_t;

typedef struct {
    ngx_http_event_handler_pt                     read_event_handler;
} ngx_media_upload_progress_module_ctx_t;

static ngx_int_t ngx_http_reportuploads_handler(ngx_http_request_t *r);
static void ngx_media_upload_progress_cleanup(void *data);
static char *ngx_media_report_uploads(ngx_conf_t * cf, ngx_command_t * cmd, void *conf);
static ngx_int_t ngx_media_upload_progress_init_zone(ngx_shm_zone_t * shm_zone, void *data);
static ngx_int_t ngx_media_upload_progress_init(ngx_conf_t * cf);
static void *ngx_media_upload_progress_create_loc_conf(ngx_conf_t *cf);
static char *ngx_media_upload_progress_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child);
static ngx_int_t ngx_media_upload_progress_init_variables_and_templates(ngx_conf_t *cf);

static ngx_int_t ngx_media_upload_progress_received_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_media_upload_progress_offset_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_media_upload_progress_status_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_media_upload_progress_callback_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static char* ngx_media_upload_progress_set_template(ngx_conf_t * cf, ngx_media_upload_progress_template_t *t, ngx_str_t *source);
static char *ngx_media_track_uploads(ngx_conf_t * cf, ngx_command_t * cmd, void *conf);
static char *ngx_media_report_uploads(ngx_conf_t * cf, ngx_command_t * cmd, void *conf);
static char *ngx_media_upload_progress(ngx_conf_t * cf, ngx_command_t * cmd, void *conf);
static char* ngx_media_upload_progress_template(ngx_conf_t * cf, ngx_command_t * cmd, void *conf);
static char* ngx_media_upload_progress_java_output(ngx_conf_t * cf, ngx_command_t * cmd, void *conf);
static char* ngx_media_upload_progress_json_output(ngx_conf_t * cf, ngx_command_t * cmd, void *conf);
static char* ngx_media_upload_progress_jsonp_output(ngx_conf_t * cf, ngx_command_t * cmd, void *conf);
static void ngx_media_up_clean_old_connections(ngx_event_t * ev);
static ngx_int_t ngx_media_upload_progress_content_handler(ngx_http_request_t *r);

static ngx_http_output_header_filter_pt ngx_http_next_header_filter;

static ngx_command_t ngx_media_upload_progress_commands[] = {

    {ngx_string("upload_progress"),
     NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE2,
     ngx_media_upload_progress,
     0,
     0,
     NULL},
    ...
    {ngx_string("upload_progress_content_type"),
     ...
     NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_media_upload_progress_conf_t, content_type),
     NULL},

    {ngx_string("upload_progress_template"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE2,
     ngx_media_upload_progress_template,
     NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_media_upload_progress_conf_t, templates),
     NULL},

    {ngx_string("upload_progress_java_output"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_NOARGS,
     ngx_media_upload_progress_java_output,
     NGX_HTTP_LOC_CONF_OFFSET,
     0,
     NULL},
    ...
    {ngx_string("upload_progress_json_output"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_NOARGS,
     ngx_media_upload_progress_json_output,
     NGX_HTTP_LOC_CONF_OFFSET,
     0,
     NULL},
    ...
    {ngx_string("upload_progress_jsonp_output"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_NOARGS,
     ngx_media_upload_progress_jsonp_output,
     NGX_HTTP_LOC_CONF_OFFSET,
     0,
     NULL},
    ...
    {ngx_string("upload_progress_header"),
     ...
     ngx_conf_set_str_slot,
     NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_media_upload_progress_conf_t, header),
     NULL},
    ...
    {ngx_string("upload_progress_jsonp_parameter"),
     ...
     ngx_conf_set_str_slot,
     NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_media_upload_progress_conf_t, jsonp_parameter),
     NULL},

    ngx_null_command
};

static ngx_http_variable_t  ngx_media_upload_progress_variables[] = {

    { ngx_string("uploadprogress_received"), NULL, ngx_media_upload_progress_received_variable,
      (uintptr_t) offsetof(ngx_media_upload_progress_node_t, rest),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("uploadprogress_remaining"), NULL, ngx_media_upload_progress_offset_variable,
      (uintptr_t) offsetof(ngx_media_upload_progress_node_t, rest),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("uploadprogress_length"), NULL, ngx_media_upload_progress_offset_variable,
      (uintptr_t) offsetof(ngx_media_upload_progress_node_t, length),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("uploadprogress_status"), NULL, ngx_media_upload_progress_status_variable,
      (uintptr_t) offsetof(ngx_media_upload_progress_node_t, err_status),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("uploadprogress_callback"), NULL, ngx_media_upload_progress_callback_variable,
      (uintptr_t) NULL,
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },
...
static ngx_http_module_t         ngx_media_upload_progress_module_ctx = {
    ngx_media_upload_progress_init_variables_and_templates,      /* preconfiguration */
    ngx_media_upload_progress_init,                              /* postconfiguration */
    ...
    ngx_media_upload_progress_create_loc_conf,                   /* create location configuration */
    ngx_media_upload_progress_merge_loc_conf                     /* merge location configuration */
};


ngx_module_t                     ngx_media_upload_progress_module = {
    NGX_MODULE_V1,
    &ngx_media_upload_progress_module_ctx,        /* module context */
    ngx_media_upload_progress_commands,           /* module directives */
...
static ngx_media_upload_progress_state_map_t ngx_media_upload_progress_state_map[] = {
...
static ngx_str_t ngx_media_upload_progress_java_defaults[] = {
...
static ngx_str_t ngx_media_upload_progress_json_defaults[] = {
...
static ngx_str_t ngx_media_upload_progress_jsonp_defaults[] = {
...
static ngx_array_t ngx_media_upload_progress_global_templates;

static ngx_str_t*
ngx_media_uprogress_get_tracking_id(ngx_http_request_t * r)
{
    u_char                          *p, *start_p;
    ngx_uint_t                       i;
    ...
    ngx_str_t                       *ret, args;
    ngx_media_upload_progress_conf_t  *upcf;

    upcf = ngx_http_get_module_loc_conf(r, ngx_media_upload_progress_module);

    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "upload-progress: ngx_media_uprogress_get_tracking_id");
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        if (header[i].key.len == upcf->header.len
            ...
            ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                            "upload-progress: ngx_media_uprogress_get_tracking_id found header: %V", ret);
            return ret;
        }
    }

    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                    "upload-progress: ngx_media_uprogress_get_tracking_id no header found");
    ...
    if (args.len && args.data) {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "upload-progress: ngx_media_uprogress_get_tracking_id no header found, args found");
        i = 0;
        p = args.data;
        do {
            ...
            if (len >= (upcf->header.len + 1) && ngx_strncasecmp(p, upcf->header.data, upcf->header.len) == 0
                && p[upcf->header.len] == '=') {
              ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                             "upload-progress: ngx_media_uprogress_get_tracking_id found args: %s",p);
                i = 1;
                break;
            }
            if (len<=0)
        ...
        if (i) {
            ...
            ret->len = p - start_p;
            ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           "upload-progress: ngx_media_uprogress_get_tracking_id found args: %V",ret);
            return ret;
        }
    }

    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "upload-progress: ngx_media_uprogress_get_tracking_id no id found");
    return NULL;
}

static ngx_media_upload_progress_node_t *
ngx_media_uprogress_find_node(ngx_str_t * id, ngx_media_upload_progress_ctx_t * ctx, ngx_log_t * log)
{
    uint32_t                         hash;
    ngx_rbtree_node_t               *node, *sentinel;
    ngx_int_t                        rc;
    ngx_media_upload_progress_node_t  *up;

    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, log, 0, "upload-progress: ngx_media_uprogress_find_node %V", id);
    ...
    while (node != sentinel) {
        ...
        do {
            up = (ngx_media_upload_progress_node_t *) node;
...
static void ngx_media_upload_progress_event_handler(ngx_http_request_t *r);

static ngx_int_t
ngx_media_upload_progress_content_handler(ngx_http_request_t *r)
{
    ngx_int_t                                    rc;
    ngx_media_upload_progress_module_ctx_t        *ctx;
    ngx_media_upload_progress_conf_t              *upcf;

    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "upload-progress: ngx_media_upload_progress_content_handler");
    upcf = ngx_http_get_module_loc_conf(r, ngx_media_upload_progress_module);
    ...
    ctx = ngx_http_get_module_ctx(r, ngx_media_upload_progress_module);
    if (ctx != NULL) {
        ctx->read_event_handler = r->read_event_handler;
        r->read_event_handler = ngx_media_upload_progress_event_handler;
    }
    return rc;
}

static ngx_str_t* ngx_media_upload_progress_strdup(ngx_str_t *src,  ngx_log_t * log)
{
    ngx_str_t *dst;
...
static void ngx_media_upload_progress_strdupfree(ngx_str_t *str)
{
    ngx_free(str);
}
...
static void ngx_media_upload_progress_event_handler(ngx_http_request_t *r)
{
    ...
    ngx_shm_zone_t                              *shm_zone;
    ngx_media_upload_progress_ctx_t               *ctx;
    ngx_media_upload_progress_node_t              *up;
    ngx_media_upload_progress_conf_t              *upcf;
    ngx_media_upload_progress_module_ctx_t        *module_ctx;
    size_t                                       size;
    off_t                                        rest;


    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "upload-progress: ngx_media_upload_progress_event_handler");

    /* find node, update rest */
    oldid = id = ngx_media_uprogress_get_tracking_id(r);
    ...
    id = ngx_media_upload_progress_strdup(id, r->connection->log);
    ngx_free(oldid);
    ...
    module_ctx = ngx_http_get_module_ctx(r, ngx_media_upload_progress_module);
    if (module_ctx != NULL ) {
    ...
    if (shm_zone == NULL) {
        ngx_media_upload_progress_strdupfree(id);
    ...
    up = ngx_media_uprogress_find_node(id, ctx, ngx_cycle->log);
    if (up != NULL && !up->done) {
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
...
ngx_http_reportuploads_handler(ngx_http_request_t * r)
{
    ...
    ngx_media_upload_progress_state_t  state;
    ngx_media_upload_progress_template_t  *t;
    ...
    id = ngx_media_uprogress_get_tracking_id(r);
    ...
    up = ngx_media_uprogress_find_node(id, ctx, r->connection->log);
    if (up != NULL) {
    ...
    ngx_http_set_ctx(r, up, ngx_media_upload_progress_module);
...
*/
static ngx_int_t
ngx_media_upload_progress_handler(ngx_http_request_t * r)
{
    size_t                           n;
    ngx_str_t                       *id;
    uint32_t                         hash;
    ...
    ngx_media_upload_progress_ctx_t   *ctx;
    ngx_media_upload_progress_node_t  *up;
    ngx_media_upload_progress_cleanup_t *upcln;
    ...
    if (ngx_media_uprogress_find_node(id, ctx, r->connection->log) != NULL) {
        ngx_shmtx_unlock(&shpool->mutex);
    ...
    cln = ngx_pool_cleanup_add(r->pool, sizeof(ngx_media_upload_progress_cleanup_t));
    if (cln == NULL) {
        ngx_shmtx_unlock(&shpool->mutex);
        ngx_free(id);
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    ...
    n = sizeof(ngx_media_upload_progress_node_t)
        + id->len;
    ...
    if (r->headers_in.content_length_n) {
        up->length = r->headers_in.content_length_n;
        up->rest   = r->headers_in.content_length_n;
        if (r->request_body) {
    ...
    if (!upcf->cleanup.timer_set) {
        ...
        upcf->cleanup.handler = ngx_media_up_clean_old_connections;
    ...
    cln->handler = ngx_media_upload_progress_cleanup;
    upcln = cln->data;
    ...
    ctx = ngx_pcalloc(r->pool, sizeof(ngx_media_upload_progress_module_ctx_t));
    if (ctx == NULL) {
      return NGX_ERROR;
    }

    ngx_http_set_ctx(r, ctx, ngx_media_upload_progress_module);
...
static void
ngx_media_upload_progress_rbtree_insert_value(ngx_rbtree_node_t * temp,
                                            ngx_rbtree_node_t * node,
                                            ngx_rbtree_node_t * sentinel)
{
    ngx_media_upload_progress_node_t  *upn, *upnt;
    ...
    for (;;) {
        ...
        if (node->key < temp->key) {
            ...
            upn = (ngx_media_upload_progress_node_t *) node;
            upnt = (ngx_media_upload_progress_node_t *) temp;
...
static void
ngx_media_up_clean_old_connections(ngx_event_t * ev)
{
    ngx_shm_zone_t                  *shm_zone;
    ngx_media_upload_progress_ctx_t   *ctx;
    ngx_slab_pool_t                 *shpool;
    ngx_rbtree_node_t               *node;
    ngx_media_upload_progress_node_t  *up, *upprev;
    time_t                           now = ngx_time();
    int                              count = 0;
...
*/
static void
ngx_media_upload_progress_cleanup(void *data)
{
    ngx_media_upload_progress_cleanup_t *upcln = data;
    ngx_slab_pool_t                 *shpool;
    ngx_rbtree_node_t               *node;
    ngx_media_upload_progress_node_t  *up;
    ngx_http_request_t              *r;
...
static ngx_int_t
ngx_media_upload_progress_init_zone(ngx_shm_zone_t * shm_zone, void *data)
{
    ngx_media_upload_progress_ctx_t   *octx = data;
    ...
    ctx->rbtree->insert = ngx_media_upload_progress_rbtree_insert_value;

    return NGX_OK;
}

static ngx_int_t
ngx_media_upload_progress_errortracker(ngx_http_request_t * r)
{
    size_t                           n;
    ngx_str_t                       *id;
    ngx_slab_pool_t                 *shpool;
    ...
    if (r->err_status >= NGX_HTTP_SPECIAL_RESPONSE) {
        ...
        if ((up = ngx_media_uprogress_find_node(id, ctx, r->connection->log)) != NULL) {
            ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
        ...
        n = sizeof(ngx_media_upload_progress_node_t) + id->len;
...
static ngx_int_t
ngx_media_upload_progress_init(ngx_conf_t * cf)
{
    ...
    *h = ngx_media_upload_progress_handler;
    ...
    ngx_http_top_header_filter = ngx_media_upload_progress_errortracker;

    return NGX_OK;
}

static void*
ngx_media_upload_progress_create_loc_conf(ngx_conf_t * cf)
{
    ngx_media_upload_progress_conf_t  *conf;
    ngx_uint_t                            i;

    conf = ngx_pcalloc(cf->pool, sizeof(ngx_media_upload_progress_conf_t));
    if (conf == NULL) {
        return NGX_CONF_ERROR;
    }

    if(ngx_array_init(&conf->templates, cf->pool, 4, sizeof(ngx_media_upload_progress_template_t)) != NGX_OK) {
        return NGX_CONF_ERROR;
    }

    for(i = 0;i < conf->templates.nalloc; i++) {
        ngx_media_upload_progress_template_t *elt = ngx_array_push(&conf->templates);
        if (elt == NULL) {
            return NGX_CONF_ERROR;
        }
...
static char*
ngx_media_upload_progress_merge_loc_conf(ngx_conf_t * cf, void *parent, void *child)
{
    ngx_media_upload_progress_conf_t  *prev = parent;
    ngx_media_upload_progress_conf_t  *conf = child;
    ngx_media_upload_progress_template_t   *t, *pt, *gt;
    ngx_uint_t                            i;
    ...
    gt = ngx_media_upload_progress_global_templates.elts;
...
static ngx_int_t
ngx_media_upload_progress_init_variables_and_templates(ngx_conf_t *cf)
{
    ngx_http_variable_t  *var, *v;
    ngx_media_upload_progress_state_map_t  *m;
    ngx_uint_t                            i;

    /* Add variables */
    for (v = ngx_media_upload_progress_variables; v->name.len; v++) {
    ...
    if(ngx_array_init(&ngx_media_upload_progress_global_templates, cf->pool, 4,
        sizeof(ngx_media_upload_progress_template_t)) != NGX_OK) {
        return NGX_ERROR;
    }

    m = ngx_media_upload_progress_state_map;
    i = 0;

    while(m->name.data != NULL) {
        ngx_media_upload_progress_template_t *elt = ngx_array_push(&ngx_media_upload_progress_global_templates);
        ngx_http_script_variables_count(ngx_media_upload_progress_jsonp_defaults + i);

        if (ngx_media_upload_progress_set_template(cf, elt, ngx_media_upload_progress_jsonp_defaults + i) != NGX_CONF_OK) {
            return NGX_ERROR;
        }
...
static char*
ngx_media_upload_progress(ngx_conf_t * cf, ngx_command_t * cmd, void *conf)
{
    ssize_t                          n;
    ngx_str_t                       *value;
    ngx_shm_zone_t                  *shm_zone;
    ...
    ctx = ngx_pcalloc(cf->pool, sizeof(ngx_media_upload_progress_ctx_t));
    if (ctx == NULL) {
        return NGX_CONF_ERROR;
    }
    ...
    shm_zone->init = ngx_media_upload_progress_init_zone;
    shm_zone->data = ctx;
...
static char*
ngx_media_track_uploads(ngx_conf_t * cf, ngx_command_t * cmd, void *conf)
{
    ngx_http_core_loc_conf_t        *clcf;
    ngx_media_upload_progress_conf_t  *lzcf = conf;
    ngx_str_t                       *value;
    ...
    clcf->handler = ngx_media_upload_progress_content_handler;
    return NGX_CONF_OK;
}


static char*
ngx_media_report_uploads(ngx_conf_t * cf, ngx_command_t * cmd, void *conf)
{
    ngx_media_upload_progress_conf_t  *lzcf = conf;
    ngx_http_core_loc_conf_t        *clcf;
    ngx_str_t                       *value;
...
static char*
ngx_media_upload_progress_set_template(ngx_conf_t * cf, ngx_media_upload_progress_template_t *t, ngx_str_t *source)
{
    ssize_t                               n;
...
static char*
ngx_media_upload_progress_template(ngx_conf_t * cf, ngx_command_t * cmd, void *conf)
{
    ngx_media_upload_progress_conf_t       *upcf = conf;
    ngx_str_t                            *value;
    ngx_media_upload_progress_state_map_t  *m = ngx_media_upload_progress_state_map;
    ngx_media_upload_progress_template_t   *t;
    ...
    t = (ngx_media_upload_progress_template_t*)upcf->templates.elts + (ngx_uint_t)m->idx;

    return ngx_media_upload_progress_set_template(cf, t, &value[2]);
}

static char*
ngx_media_upload_progress_java_output(ngx_conf_t * cf, ngx_command_t * cmd, void *conf)
{
    ngx_media_upload_progress_conf_t       *upcf = conf;
    ngx_media_upload_progress_template_t   *t;
    ngx_uint_t                            i;
    ...
    t = (ngx_media_upload_progress_template_t*)upcf->templates.elts;

    for(i = 0;i < upcf->templates.nelts;i++) {
        rc = ngx_media_upload_progress_set_template(cf, t + i, ngx_media_upload_progress_java_defaults + i);
...
static char*
ngx_media_upload_progress_json_output(ngx_conf_t * cf, ngx_command_t * cmd, void *conf)
{
    ngx_media_upload_progress_conf_t       *upcf = conf;
    ngx_media_upload_progress_template_t   *t;
    ngx_uint_t                            i;
    ...
    for(i = 0;i < upcf->templates.nelts;i++) {
        rc = ngx_media_upload_progress_set_template(cf, t + i, ngx_media_upload_progress_json_defaults + i);
...
static char*
ngx_media_upload_progress_jsonp_output(ngx_conf_t * cf, ngx_command_t * cmd, void *conf)
{
    ngx_media_upload_progress_conf_t       *upcf = conf;
    ngx_media_upload_progress_template_t   *t;
    ngx_uint_t                            i;
    ...
    for(i = 0;i < upcf->templates.nelts;i++) {
        rc = ngx_media_upload_progress_set_template(cf, t + i, ngx_media_upload_progress_jsonp_defaults + i);
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    up = ngx_http_get_module_ctx(r, ngx_media_upload_progress_module);
...
static ngx_int_t
ngx_media_upload_progress_status_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_media_upload_progress_node_t  *up;
    u_char                          *p;
...
static ngx_int_t
ngx_media_upload_progress_callback_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
