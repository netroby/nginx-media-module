...
#include "ngx_media_upload_module.h"
...
typedef struct {
    ngx_buf_t               *in_buf;
    ngx_buf_t               *out_buf;
    ngx_media_upload_range_t  current_range_n;
    off_t                   *parser_state;
    ngx_log_t               *log;
...
} ngx_media_upload_merger_state_t;
...
} ngx_media_upload_field_template_t;
...
} ngx_media_upload_header_template_t;
...
typedef struct {
    ...
    ngx_str_t                         text;
#endif
} ngx_media_upload_field_filter_t;

typedef struct {
    ngx_path_t                       *path;
    ngx_http_complex_value_t          dynamic;
    unsigned                          is_dynamic:1;
} ngx_media_upload_path_t;

/*
 * Upload cleanup record
 */
typedef struct ngx_media_upload_cleanup_s {
    ngx_fd_t                          fd;
    u_char                           *filename;
...
typedef struct {
    ...
    ngx_media_upload_path_t        *state_store_path;
    ngx_media_upload_path_t        *store_path;
...
} ngx_media_upload_loc_conf_t;

typedef struct ngx_media_upload_md5_ctx_s {
    MD5_CTX     md5;
    u_char      md5_digest[MD5_DIGEST_LENGTH * 2];
} ngx_media_upload_md5_ctx_t;

typedef struct ngx_media_upload_sha1_ctx_s {
    SHA_CTX     sha1;
    u_char      sha1_digest[SHA_DIGEST_LENGTH * 2];
} ngx_media_upload_sha1_ctx_t;

typedef struct ngx_media_upload_sha256_ctx_s {
    SHA256_CTX  sha256;
    u_char      sha256_digest[SHA256_DIGEST_LENGTH * 2];
} ngx_media_upload_sha256_ctx_t;

typedef struct ngx_media_upload_sha512_ctx_s {
    SHA512_CTX  sha512;
    u_char      sha512_digest[SHA512_DIGEST_LENGTH * 2];
} ngx_media_upload_sha512_ctx_t;

struct ngx_media_upload_ctx_s;

/*
 * Request body data handler
 */
typedef ngx_int_t (*ngx_http_request_body_data_handler_pt)
    (struct ngx_media_upload_ctx_s*, u_char *, u_char*);

/*
 * Upload module context
 */
typedef struct ngx_media_upload_ctx_s {
    ...
    ngx_media_upload_range_t       content_range_n;
    ...
    ngx_int_t (*start_part_f)(struct ngx_media_upload_ctx_s *upload_ctx);
    void (*finish_part_f)(struct ngx_media_upload_ctx_s *upload_ctx);
    void (*abort_part_f)(struct ngx_media_upload_ctx_s *upload_ctx);
	ngx_int_t (*flush_output_buffer_f)(struct ngx_media_upload_ctx_s *upload_ctx, u_char *buf, size_t len);

    ngx_http_request_t                 *request;
    ngx_log_t                          *log;

    ngx_file_t                          output_file;
    ngx_str_t                           output_name;
    ngx_str_t                           output_prefix;
    ...
    ngx_media_upload_md5_ctx_t    *md5_ctx;
    ngx_media_upload_sha1_ctx_t   *sha1_ctx;
    ngx_media_upload_sha256_ctx_t *sha256_ctx;
    ngx_media_upload_sha512_ctx_t *sha512_ctx;
    uint32_t                            crc32;
...
} ngx_media_upload_ctx_t;

static ngx_int_t ngx_media_upload_test_expect(ngx_http_request_t *r);

static ngx_int_t ngx_media_upload_handler(ngx_http_request_t *r);
static ngx_int_t ngx_media_upload_options_handler(ngx_http_request_t *r);
static ngx_int_t ngx_media_upload_body_handler(ngx_http_request_t *r);

static void     *ngx_media_upload_create_loc_conf(ngx_conf_t *cf);
static char     *ngx_media_upload_merge_loc_conf(ngx_conf_t *cf,void *parent, void *child);
static ngx_int_t ngx_media_upload_add_variables(ngx_conf_t *cf);
static void      ngx_media_upload_variable_set(ngx_http_request_t *r,ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_media_upload_variable(ngx_http_request_t *r,ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_media_upload_md5_variable(ngx_http_request_t *r,ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_media_upload_sha1_variable(ngx_http_request_t *r,ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_media_upload_sha256_variable(ngx_http_request_t *r,ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_media_upload_sha512_variable(ngx_http_request_t *r,ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_media_upload_file_size_variable(ngx_http_request_t *r,ngx_http_variable_value_t *v, uintptr_t data);
static void      ngx_media_upload_content_range_variable_set(ngx_http_request_t *r,ngx_http_variable_value_t *v,  uintptr_t data);
static ngx_int_t ngx_media_upload_content_range_variable(ngx_http_request_t *r,ngx_http_variable_value_t *v,  uintptr_t data);
static ngx_int_t ngx_media_upload_crc32_variable(ngx_http_request_t *r,ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_media_upload_uint_variable(ngx_http_request_t *r,ngx_http_variable_value_t *v, uintptr_t data);
static char     *ngx_media_upload_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);

static ngx_int_t ngx_media_upload_start_handler(ngx_media_upload_ctx_t *u);
static void      ngx_media_upload_finish_handler(ngx_media_upload_ctx_t *u);
static void      ngx_media_upload_abort_handler(ngx_media_upload_ctx_t *u);

static ngx_int_t ngx_media_upload_flush_output_buffer(ngx_media_upload_ctx_t *u,u_char *buf, size_t len);
static ngx_int_t ngx_media_upload_append_field(ngx_media_upload_ctx_t *u,ngx_str_t *name, ngx_str_t *value);
static ngx_int_t ngx_media_upload_merge_ranges(ngx_media_upload_ctx_t *u, ngx_media_upload_range_t *range_n);
static ngx_int_t ngx_media_upload_parse_range(ngx_str_t *range, ngx_media_upload_range_t *range_n);
...
static char     *ngx_media_upload_set_form_field(ngx_conf_t *cf, ngx_command_t *cmd,void *conf);
static char     *ngx_media_upload_add_header(ngx_conf_t *cf, ngx_command_t *cmd,void *conf);
static ngx_int_t ngx_media_upload_eval_path(ngx_http_request_t *r);
static ngx_int_t ngx_media_upload_eval_state_path(ngx_http_request_t *r);
static char     *ngx_media_upload_pass_form_field(ngx_conf_t *cf, ngx_command_t *cmd,void *conf);
static char     *ngx_media_upload_set_path_slot(ngx_conf_t *cf, ngx_command_t *cmd,void *conf);
static char     *ngx_media_upload_merge_path_value(ngx_conf_t *cf, ngx_media_upload_path_t **path, ngx_media_upload_path_t *prev,ngx_path_init_t *init);
static char     *ngx_media_upload_cleanup(ngx_conf_t *cf, ngx_command_t *cmd,void *conf);
static void      ngx_upload_cleanup_handler(void *data);

#if defined nginx_version && nginx_version >= 7052
static ngx_path_init_t        ngx_media_upload_temp_path = {
 ...
 *
 */
static void upload_init_ctx(ngx_media_upload_ctx_t *upload_ctx);
 ...
 *
 */
static void upload_shutdown_ctx(ngx_media_upload_ctx_t *upload_ctx);
 ...
 *
 */
static ngx_int_t upload_start(ngx_media_upload_ctx_t *upload_ctx, ngx_media_upload_loc_conf_t  *ulcf);
 ...
 *     NGX_ERROR if error has occured
 */
static ngx_int_t upload_parse_request_headers(ngx_media_upload_ctx_t *upload_ctx, ngx_http_headers_in_t *headers_in);
 ...
 */
static ngx_int_t upload_process_buf(ngx_media_upload_ctx_t *upload_ctx, u_char *start, u_char *end);
static ngx_int_t upload_process_raw_buf(ngx_media_upload_ctx_t *upload_ctx, u_char *start, u_char *end);

static void      ngx_media_upload_get_suffix(ngx_pool_t* pool,ngx_str_t* file_name,ngx_str_t* suffix);


static ngx_command_t  ngx_media_upload_commands[] = { /* {{{ */
    ...
    { ngx_string("upload_store"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_HTTP_LMT_CONF|NGX_HTTP_LIF_CONF
                        |NGX_CONF_TAKE1234,
      ngx_media_upload_set_path_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, store_path),
      NULL },
    ...
    { ngx_string("upload_state_store"),
      ...
      ngx_media_upload_set_path_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, state_store_path),
      NULL },
    ...
    { ngx_string("upload_store_access"),
      ...
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, store_access),
      NULL },
    ...
    { ngx_string("upload_buffer_size"),
      ...
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, buffer_size),
      NULL },
    ...
    { ngx_string("upload_merge_buffer_size"),
      ...
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, merge_buffer_size),
      NULL },
    ...
    { ngx_string("upload_range_header_buffer_size"),
      ...
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, range_header_buffer_size),
      NULL },
    ...
    { ngx_string("upload_max_part_header_len"),
      ...
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, max_header_len),
      NULL },
    ...
    { ngx_string("upload_max_file_size"),
      ...
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, max_file_size),
      NULL },
    ...
    { ngx_string("upload_max_output_body_len"),
      ...
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, max_output_body_len),
      NULL },
    ...
    { ngx_string("upload_set_form_field"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_HTTP_LMT_CONF|NGX_HTTP_LIF_CONF
                        |NGX_CONF_TAKE2,
      ngx_media_upload_set_form_field,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, field_templates),
      NULL},
    ...
    { ngx_string("upload_aggregate_form_field"),
      ...
      ngx_media_upload_set_form_field,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, aggregate_field_templates),
      NULL},
    ...
    { ngx_string("upload_pass_form_field"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_HTTP_LMT_CONF|NGX_HTTP_LIF_CONF
                        |NGX_CONF_TAKE1,
      ngx_media_upload_pass_form_field,
      NGX_HTTP_LOC_CONF_OFFSET,
      0,
      NULL},
    ...
    { ngx_string("upload_cleanup"),
     ...
     { ngx_string("upload_pass_args"),
       ...
       NGX_HTTP_LOC_CONF_OFFSET,
       offsetof(ngx_media_upload_loc_conf_t, forward_args),
       NULL },
    ...
    { ngx_string("upload_limit_rate"),
      ...
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, limit_rate),
      NULL },

      /*
      * Specifies add the suffix to file name
      */
     { ngx_string("upload_add_suffix"),
       NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_HTTP_LMT_CONF|NGX_HTTP_LIF_CONF
                         |NGX_CONF_FLAG,
       ngx_conf_set_flag_slot,
       NGX_HTTP_LOC_CONF_OFFSET,
       offsetof(ngx_media_upload_loc_conf_t, add_suffix),
       NULL },
     ...
     { ngx_string("upload_tame_arrays"),
       ...
       ngx_conf_set_flag_slot,
       NGX_HTTP_LOC_CONF_OFFSET,
       offsetof(ngx_media_upload_loc_conf_t, tame_arrays),
       NULL },
     ...
     { ngx_string("upload_resumable"),
       ...
       ngx_conf_set_flag_slot,
       NGX_HTTP_LOC_CONF_OFFSET,
       offsetof(ngx_media_upload_loc_conf_t, resumable_uploads),
       NULL },
     ...
     { ngx_string("upload_empty_fiels_names"),
       ...
       ngx_conf_set_flag_slot,
       NGX_HTTP_LOC_CONF_OFFSET,
       offsetof(ngx_media_upload_loc_conf_t, empty_field_names),
       NULL },
    ...
    { ngx_string("upload_add_header"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_HTTP_LMT_CONF|NGX_HTTP_LIF_CONF
                        |NGX_CONF_TAKE2,
      ngx_media_upload_add_header,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, header_templates),
      NULL},

      ngx_null_command
}; /* }}} */

ngx_http_module_t  ngx_media_upload_module_ctx = { /* {{{ */
    ngx_media_upload_add_variables,         /* preconfiguration */
    ...
    ngx_media_upload_create_loc_conf,       /* create location configuration */
    ngx_media_upload_merge_loc_conf         /* merge location configuration */
}; /* }}} */

ngx_module_t  ngx_media_upload_module = { /* {{{ */
    NGX_MODULE_V1,
    &ngx_media_upload_module_ctx,           /* module context */
    ngx_media_upload_commands,              /* module directives */
...
static ngx_http_variable_t  ngx_media_upload_variables[] = { /* {{{ */

    { ngx_string("upload_field_name"), NULL, ngx_media_upload_variable,
      (uintptr_t) offsetof(ngx_media_upload_ctx_t, field_name),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_content_type"),
      ngx_media_upload_variable_set,
      ngx_media_upload_variable,
      (uintptr_t) offsetof(ngx_media_upload_ctx_t, content_type),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_name"), NULL, ngx_media_upload_variable,
      (uintptr_t) offsetof(ngx_media_upload_ctx_t, file_name),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_number"), NULL, ngx_media_upload_uint_variable,
      (uintptr_t) offsetof(ngx_media_upload_ctx_t, ordinal),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_tmp_path"), NULL, ngx_media_upload_variable,
      (uintptr_t) offsetof(ngx_media_upload_ctx_t, output_file.name),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_output_name"), NULL, ngx_media_upload_variable,
      (uintptr_t) offsetof(ngx_media_upload_ctx_t, output_name),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_content_range"),
      ngx_media_upload_content_range_variable_set,
      ngx_media_upload_content_range_variable,
      (uintptr_t) offsetof(ngx_media_upload_ctx_t, content_range_n),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_null_string, NULL, NULL, 0, 0, 0 }
}; /* }}} */

static ngx_http_variable_t  ngx_media_upload_aggregate_variables[] = { /* {{{ */

    { ngx_string("upload_file_md5"), NULL, ngx_media_upload_md5_variable,
      (uintptr_t) "0123456789abcdef",
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_md5_uc"), NULL, ngx_media_upload_md5_variable,
      (uintptr_t) "0123456789ABCDEF",
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_sha1"), NULL, ngx_media_upload_sha1_variable,
      (uintptr_t) "0123456789abcdef",
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_sha1_uc"), NULL, ngx_media_upload_sha1_variable,
      (uintptr_t) "0123456789ABCDEF",
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_sha256"), NULL, ngx_media_upload_sha256_variable,
      (uintptr_t) "0123456789abcdef",
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_sha256_uc"), NULL, ngx_media_upload_sha256_variable,
      (uintptr_t) "0123456789ABCDEF",
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_sha512"), NULL, ngx_media_upload_sha512_variable,
      (uintptr_t) "0123456789abcdef",
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_sha512_uc"), NULL, ngx_media_upload_sha512_variable,
      (uintptr_t) "0123456789ABCDEF",
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_crc32"), NULL, ngx_media_upload_crc32_variable,
      (uintptr_t) offsetof(ngx_media_upload_ctx_t, crc32),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_size"), NULL, ngx_media_upload_file_size_variable,
      (uintptr_t) offsetof(ngx_media_upload_ctx_t, output_file.offset),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_null_string, NULL, NULL, 0, 0, 0 }
}; /* }}} */

static ngx_str_t  ngx_media_upload_empty_field_value = ngx_null_string;
...
static ngx_int_t /* {{{ ngx_media_upload_handler */
ngx_media_upload_handler(ngx_http_request_t *r)
{
    ngx_media_upload_loc_conf_t  *ulcf;
    ngx_media_upload_ctx_t     *u;
    ngx_int_t                 rc;
    ngx_str_t                      arg;

    if(r->method & NGX_HTTP_OPTIONS)
        return ngx_media_upload_options_handler(r);

    if (!(r->method & NGX_HTTP_POST))
        return NGX_HTTP_NOT_ALLOWED;

    ulcf = ngx_http_get_module_loc_conf(r, ngx_media_upload_module);

    u = ngx_http_get_module_ctx(r, ngx_media_upload_module);

    if (u == NULL) {
        u = ngx_pcalloc(r->pool, sizeof(ngx_media_upload_ctx_t));
        if (u == NULL) {
            return NGX_HTTP_INTERNAL_SERVER_ERROR;
        }

        ngx_http_set_ctx(r, u, ngx_media_upload_module);
    }

    if(ulcf->md5) {
        if(u->md5_ctx == NULL) {
            u->md5_ctx = ngx_palloc(r->pool, sizeof(ngx_media_upload_md5_ctx_t));
            if (u->md5_ctx == NULL) {
                return NGX_HTTP_INTERNAL_SERVER_ERROR;
            }
        }
    ...
    if(ulcf->sha1) {
        if(u->sha1_ctx == NULL) {
            u->sha1_ctx = ngx_palloc(r->pool, sizeof(ngx_media_upload_sha1_ctx_t));
            if (u->sha1_ctx == NULL) {
                return NGX_HTTP_INTERNAL_SERVER_ERROR;
            }
        }
    ...
    if(ulcf->sha256) {
        if(u->sha256_ctx == NULL) {
            u->sha256_ctx = ngx_palloc(r->pool, sizeof(ngx_media_upload_sha256_ctx_t));
            if (u->sha256_ctx == NULL) {
                return NGX_HTTP_INTERNAL_SERVER_ERROR;
            }
        }
    ...
    if(ulcf->sha512) {
        if(u->sha512_ctx == NULL) {
            u->sha512_ctx = ngx_palloc(r->pool, sizeof(ngx_media_upload_sha512_ctx_t));
            if (u->sha512_ctx == NULL) {
                return NGX_HTTP_INTERNAL_SERVER_ERROR;
            }
        }
    ...
    if (r->args.len) {
        if (ngx_http_arg(r, (u_char *) REQ_ARG_PREFIX, ngx_strlen(REQ_ARG_PREFIX), &arg) == NGX_OK) {
            u->output_prefix.data = arg.data;
            u->output_prefix.len= arg.len;
        }
    }
    ...
    rc = ngx_media_upload_eval_path(r);
    ...
    rc = ngx_media_upload_eval_state_path(r);
    ...
    if (ngx_media_upload_test_expect(r) != NGX_OK) {
        upload_shutdown_ctx(u);
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }
...
static ngx_int_t ngx_media_upload_add_headers(ngx_http_request_t *r, ngx_media_upload_loc_conf_t *ulcf) { /* {{{ */
    ngx_str_t                            name;
    ngx_str_t                            value;
    ngx_media_upload_header_template_t    *t;
    ngx_table_elt_t                      *h;
    ngx_uint_t                           i;
...
static ngx_int_t /* {{{  */
ngx_media_upload_eval_path(ngx_http_request_t *r) {
    ngx_media_upload_ctx_t       *u;
    ngx_media_upload_loc_conf_t  *ulcf;
    ngx_str_t                   value;
...
static ngx_int_t /* {{{  */
ngx_media_upload_eval_state_path(ngx_http_request_t *r) {
    ngx_media_upload_ctx_t       *u;
    ngx_media_upload_loc_conf_t  *ulcf;
    ngx_str_t                   value;
...
static ngx_int_t ngx_media_upload_options_handler(ngx_http_request_t *r) { /* {{{ */
    ...
    if(ngx_media_upload_add_headers(r, ulcf) != NGX_OK) {
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }
...
static ngx_int_t ngx_media_upload_body_handler(ngx_http_request_t *r) { /* {{{ */
    ngx_media_upload_loc_conf_t  *ulcf = ngx_http_get_module_loc_conf(r, ngx_media_upload_module);
    ngx_media_upload_ctx_t       *ctx = ngx_http_get_module_ctx(r, ngx_media_upload_module);
    ...
    if(ctx->no_content) {
        rc = ngx_media_upload_append_field(ctx, &dummy, &ngx_media_upload_empty_field_value);
...
static ngx_int_t ngx_media_upload_start_handler(ngx_media_upload_ctx_t *u) { /* {{{ */
    ...
    ngx_str_t   *output = &u->output_name;
    ngx_str_t   *prefix = &u->output_prefix;
    ...
    ngx_int_t   rc = NGX_OK;
    ngx_err_t   err;
    ngx_media_upload_field_template_t    *t;
    ngx_media_upload_field_filter_t    *f;
    ...
    if(u->is_file) {
        ...
        if(ulcf->add_suffix) {
            ngx_str_null(&suffix);
            ngx_media_upload_get_suffix(u->request->pool,&u->file_name,&suffix);
            if((0 != prefix->len)&&(NULL != prefix->data)) {
                file->name.len = path->name.len + 1 + path->len + prefix->len + suffix.len;
                output->len = prefix->len + suffix.len;

            }
            else if((0 != u->session_id.len)&&(NULL != u->session_id.data)) {
                file->name.len = path->name.len + 1 + path->len + u->session_id.len + suffix.len;
                output->len = u->session_id.len + suffix.len;
            }
            else {
                file->name.len = path->name.len + 1 + path->len + 24 + suffix.len;
                output->len = 24 + suffix.len;
            }

            file->name.data = ngx_palloc(u->request->pool, file->name.len + 1);
            output->data = ngx_pcalloc(u->request->pool,output->len);
            ngx_log_debug1(NGX_LOG_DEBUG_CORE, r->connection->log, 0,
                           "get upload suffix:%V", &suffix);
        }
        else {

            if((0 != prefix->len)&&(NULL != prefix->data)) {
                file->name.len = path->name.len + 1 + path->len + prefix->len;
                output->len = prefix->len;
            }
            else if((0 != u->session_id.len)&&(NULL != u->session_id.data)) {
                file->name.len = path->name.len + 1 + path->len + u->session_id.len;
                output->len = u->session_id.len ;
            }
            else {
                file->name.len = path->name.len + 1 + path->len + 24;
                output->len = 24;
            }
        ...
        if(u->session_id.len != 0) {
            if(ulcf->add_suffix) {
                (void) ngx_sprintf(file->name.data + path->name.len + 1 + path->len,
                           "%V%V%Z", &u->session_id,&suffix);
                (void) ngx_sprintf(output->data,"%V%V", &u->session_id,&suffix);
            }
            else {

                (void) ngx_sprintf(file->name.data + path->name.len + 1 + path->len,
                           "%V%Z", &u->session_id);
                (void) ngx_sprintf(output->data,"%V", &u->session_id);
            }

            ngx_create_hashed_filename(path, file->name.data, file->name.len);

            ngx_log_debug2(NGX_LOG_DEBUG_CORE, file->log, 0,
                           "hashed path: %s,out file:%s", file->name.data,output->data);
        ...
        else if(0 != prefix->len) {
            if(ulcf->add_suffix) {
                (void) ngx_sprintf(file->name.data + path->name.len + 1 + path->len,
                               "%V%V%Z", prefix,&suffix);
                (void) ngx_sprintf(output->data,"%V%V", prefix,&suffix);
            }
            else {
                (void) ngx_sprintf(file->name.data + path->name.len + 1 + path->len,
                               "%V%Z", prefix);
                (void) ngx_sprintf(output->data,"%V", prefix);
            }
            ...
            for(;;) {
                ...
                cur  = ngx_time();
                ngx_localtime(cur, &tm);
                if((ulcf->add_suffix)&&(u->is_file)) {

                    (void) ngx_sprintf(file->name.data + path->name.len + 1 + path->len,
                                       "%04d%02d%02d%02d%02d%02d%010uD%V%Z",
                                       tm.tm_year,tm.tm_mon,tm.tm_mday,tm.tm_hour,tm.tm_min,tm.tm_sec,n,&suffix);
                    (void) ngx_sprintf(output->data,
                                       "%04d%02d%02d%02d%02d%02d%010uD%V",
                                       tm.tm_year,tm.tm_mon,tm.tm_mday,tm.tm_hour,tm.tm_min,tm.tm_sec,n,&suffix);
                }
                else {
                    (void) ngx_sprintf(file->name.data + path->name.len + 1 + path->len,
                                       "%04d%02d%02d%02d%02d%02d%010uD%Z",
                                       tm.tm_year,tm.tm_mon,tm.tm_mday,tm.tm_hour,tm.tm_min,tm.tm_sec,n);
                    (void) ngx_sprintf(output->data,
                                       "%04d%02d%02d%02d%02d%02d%010uD",
                                       tm.tm_year,tm.tm_mon,tm.tm_mday,tm.tm_hour,tm.tm_min,tm.tm_sec,n);
                }
        ...
        if(ulcf->field_templates) {
            ...
            for (i = 0; i < ulcf->field_templates->nelts; i++) {
                ...
                rc = ngx_media_upload_append_field(u, &field_name, &field_value);
        ...
        if(pass_field && u->field_name.len != 0) {
             ...
             * is not known until ngx_media_upload_flush_output_buffer
             * is called. We pass empty field value to simplify things.
             */
            rc = ngx_media_upload_append_field(u, &u->field_name, &ngx_media_upload_empty_field_value);
...
static void ngx_media_upload_finish_handler(ngx_media_upload_ctx_t *u) { /* {{{ */
    ngx_media_upload_field_template_t    *af;
    ...
    if(u->is_file) {
        ...
        if(u->partial_content) {
            ...
            rc = ngx_media_upload_merge_ranges(u, &u->content_range_n);
        ...
        if(ulcf->aggregate_field_templates) {
            ...
            for (i = 0; i < ulcf->aggregate_field_templates->nelts; i++) {
                ...
                rc = ngx_media_upload_append_field(u, &aggregate_field_name, &aggregate_field_value);
...
rollback:
    ngx_media_upload_abort_handler(u);
} /* }}} */
...
static void /* {{{ ngx_media_upload_append_str */
ngx_media_upload_append_str(ngx_media_upload_ctx_t *u, ngx_buf_t *b, ngx_chain_t *cl, ngx_str_t *s)
{
...
static ngx_int_t /* {{{ ngx_media_upload_append_field */
ngx_media_upload_append_field(ngx_media_upload_ctx_t *u, ngx_str_t *name, ngx_str_t *value)
{
    ngx_media_upload_loc_conf_t     *ulcf = ngx_http_get_module_loc_conf(u->request, ngx_media_upload_module);
    ...
    if(name->len > 0) {
        ...
        ngx_media_upload_append_str(u, b, cl, &boundary);

        ngx_media_upload_append_str(u, b + 1, cl + 1, &ngx_upload_field_part1);

        ngx_media_upload_append_str(u, b + 2, cl + 2, name);

        ngx_media_upload_append_str(u, b + 3, cl + 3, &ngx_upload_field_part2);

        if(value->len > 0)
            ngx_media_upload_append_str(u, b + 4, cl + 4, value);
...
static ngx_int_t ngx_media_upload_add_range(ngx_media_upload_merger_state_t *ms, ngx_media_upload_range_t *range_n) {
...
static ngx_int_t /* {{{ ngx_media_upload_buf_merge_range */
ngx_media_upload_buf_merge_range(ngx_media_upload_merger_state_t *ms, ngx_media_upload_range_t *range_n) {
    u_char *p, c;
    off_t                  *field;
        ...
        while(p != ms->in_buf->last) {
            ...
            if(c >= '0' && c <= '9') {
                ...
                if(ms->current_range_n.end + 1 < range_n->start) {
                     ...
                     */
                    if(ngx_media_upload_add_range(ms, &ms->current_range_n) != NGX_OK) {
                        return NGX_ERROR;
                    }
                ...
                if(ms->current_range_n.start > range_n->end + 1) {
                    ...
                    if(!ms->found_lower_bound) {
                        if(ngx_media_upload_add_range(ms, range_n) != NGX_OK) {
                            return NGX_ERROR;
                        }
                    }
...
static ngx_int_t /* {{{ ngx_media_upload_merge_ranges */
ngx_media_upload_merge_ranges(ngx_media_upload_ctx_t *u, ngx_media_upload_range_t *range_n) {
    ngx_file_t  *state_file = &u->state_file;
    ngx_media_upload_merger_state_t ms;
    off_t        remaining;
    ssize_t      rc;
    ...
    ngx_buf_t    out_buf;
    ngx_media_upload_loc_conf_t  *ulcf = ngx_http_get_module_loc_conf(u->request, ngx_media_upload_module);
    ngx_media_upload_range_t  range_to_merge_n;
    ...
    do {
        ...
        if(ngx_media_upload_buf_merge_range(&ms, &range_to_merge_n) != NGX_OK) {
...
static void * /* {{{ ngx_media_upload_create_loc_conf */
ngx_media_upload_create_loc_conf(ngx_conf_t *cf)
{
    ngx_media_upload_loc_conf_t  *conf;

    conf = ngx_pcalloc(cf->pool, sizeof(ngx_media_upload_loc_conf_t));
    if (conf == NULL) {
    ...
    conf->add_suffix = NGX_CONF_UNSET;
...
static char * /* {{{ ngx_media_upload_merge_loc_conf */
ngx_media_upload_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ngx_media_upload_loc_conf_t  *prev = parent;
    ngx_media_upload_loc_conf_t  *conf = child;
    ...
    if(conf->url.len != 0) {
        ngx_media_upload_merge_path_value(cf,
                                  &conf->store_path,
                                  prev->store_path,
                                  &ngx_media_upload_temp_path);
    ...
    if(conf->add_suffix == NGX_CONF_UNSET) {
        conf->add_suffix = (prev->add_suffix != NGX_CONF_UNSET) ?
            prev->add_suffix : 0;
    }
...
static ngx_int_t /* {{{ ngx_media_upload_add_variables */
ngx_media_upload_add_variables(ngx_conf_t *cf)
{
    ngx_http_variable_t  *var, *v;

    for (v = ngx_media_upload_variables; v->name.len; v++) {
    ...
    for (v = ngx_media_upload_aggregate_variables; v->name.len; v++) {
        var = ngx_http_add_variable(cf, &v->name, v->flags);
        if (var == NULL) {
            return NGX_ERROR;
        }
...
static void /* {{{ ngx_media_upload_variable_set */
ngx_media_upload_variable_set(ngx_http_request_t *r,
...
static ngx_int_t /* {{{ ngx_media_upload_variable */
ngx_media_upload_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ngx_media_upload_ctx_t  *u;
    ngx_str_t              *value;
...
static ngx_int_t /* {{{ ngx_media_upload_md5_variable */
ngx_media_upload_md5_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_uint_t             i;
    ngx_media_upload_ctx_t  *u;
...
static ngx_int_t /* {{{ ngx_media_upload_sha1_variable */
ngx_media_upload_sha1_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_uint_t             i;
    ngx_media_upload_ctx_t  *u;
...
static ngx_int_t /* {{{ ngx_media_upload_sha256_variable */
ngx_media_upload_sha256_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_uint_t             i;
    ngx_media_upload_ctx_t  *u;
...
static ngx_int_t /* {{{ ngx_media_upload_sha512_variable */
ngx_media_upload_sha512_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_uint_t             i;
    ngx_media_upload_ctx_t  *u;
...
static ngx_int_t /* {{{ ngx_media_upload_crc32_variable */
ngx_media_upload_crc32_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_media_upload_ctx_t  *u;
    u_char                 *p;
...
static ngx_int_t /* {{{ ngx_media_upload_file_size_variable */
ngx_media_upload_file_size_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_media_upload_ctx_t  *u;
    u_char                 *p;
...
static void /* {{{ ngx_media_upload_content_range_variable_set */
ngx_media_upload_content_range_variable_set(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_media_upload_ctx_t   *u;
    ngx_str_t                val;
    ngx_media_upload_range_t *value;

    u = ngx_http_get_module_ctx(r, ngx_media_upload_module);

    value = (ngx_media_upload_range_t *) ((char *) u + data);

    val.len = v->len;
    val.data = v->data;

    if(ngx_media_upload_parse_range(&val, value) != NGX_OK) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "invalid range \"%V\"", &val);
    }
} /* }}} */

static ngx_int_t /* {{{ ngx_media_upload_content_range_variable */
ngx_media_upload_content_range_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_media_upload_ctx_t  *u;
    u_char                 *p;
...
static ngx_int_t /* {{{ ngx_media_upload_uint_variable */
ngx_media_upload_uint_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_media_upload_ctx_t  *u;
    u_char                 *p;
...
static char * /* {{{ ngx_media_upload_set_form_field */
ngx_media_upload_set_form_field(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_int_t                   n, i;
    ngx_str_t                  *value;
    ngx_http_script_compile_t   sc;
    ngx_media_upload_field_template_t *h;
    ngx_array_t                 **field;
    ngx_http_variable_t         *v;
    u_char                      *match;
    ngx_media_upload_loc_conf_t  *ulcf = conf;
    ...
    if (*field == NULL) {
        *field = ngx_array_create(cf->pool, 1,
                                  sizeof(ngx_media_upload_field_template_t));
        if (*field == NULL) {
            return NGX_CONF_ERROR;
        }
    }
    ...
    for(i = 1;i <= 2;i++) {
        for (v = ngx_media_upload_aggregate_variables; v->name.len; v++) {
            ...
            {
                if(cmd->offset != offsetof(ngx_media_upload_loc_conf_t, aggregate_field_templates)) {
                ...
                if(v->get_handler == ngx_media_upload_md5_variable)
                    ulcf->md5 = 1;

                if(v->get_handler == ngx_media_upload_sha1_variable)
                    ulcf->sha1 = 1;

                if(v->get_handler == ngx_media_upload_sha256_variable)
                    ulcf->sha256 = 1;

                if(v->get_handler == ngx_media_upload_sha512_variable)
                    ulcf->sha512 = 1;

                if(v->get_handler == ngx_media_upload_crc32_variable)
                    ulcf->crc32 = 1;
            }
        }
    }
...
static char * /* {{{ ngx_media_upload_pass_form_field */
ngx_media_upload_pass_form_field(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_media_upload_loc_conf_t *ulcf = conf;
    ...
    if (ulcf->field_filters == NULL) {
        ulcf->field_filters = ngx_array_create(cf->pool, 1,
                                        sizeof(ngx_media_upload_field_filter_t));
        if (ulcf->field_filters == NULL) {
            return NGX_CONF_ERROR;
        }
    }
...
static char * /* {{{ ngx_media_upload_add_header */
ngx_media_upload_add_header(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_str_t                  *value;
    ngx_media_upload_header_template_t *h;
    ngx_array_t                 **field;
    ...
    if (*field == NULL) {
        *field = ngx_array_create(cf->pool, 1,
                                  sizeof(ngx_media_upload_header_template_t));
        if (*field == NULL) {
            return NGX_CONF_ERROR;
        }
    }
...
static char * /* {{{ ngx_media_upload_cleanup */
ngx_media_upload_cleanup(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_media_upload_loc_conf_t *ulcf = conf;
...
static char * /* {{{ ngx_media_upload_pass */
ngx_media_upload_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    clcf->handler = ngx_media_upload_handler;
...
static char * /* {{{ ngx_media_upload_set_path_slot */
ngx_media_upload_set_path_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    char  *p = conf;
    ...
    ngx_str_t   *value;
    ngx_uint_t   i, n;
    ngx_media_upload_path_t *path, **slot;
    ngx_http_compile_complex_value_t   ccv;

    slot = (ngx_media_upload_path_t **) (p + cmd->offset);

    if (*slot) {
        return "is duplicate";
    }

    path = ngx_pcalloc(cf->pool, sizeof(ngx_media_upload_path_t));
    if (path == NULL) {
        return NGX_CONF_ERROR;
    }
...
static char * /* {{{ ngx_media_upload_merge_path_value */
ngx_media_upload_merge_path_value(ngx_conf_t *cf, ngx_media_upload_path_t **path, ngx_media_upload_path_t *prev,
...
ngx_http_read_upload_client_request_body(ngx_http_request_t *r) {
    ...
    ngx_http_core_loc_conf_t  *clcf;
    ngx_media_upload_ctx_t     *u = ngx_http_get_module_ctx(r, ngx_media_upload_module);
    ...
    if (preread) {
        ...
        if (preread >= r->headers_in.content_length_n) {
            ...
            return ngx_media_upload_body_handler(r);
        }
...
static ngx_int_t upload_parse_content_disposition(ngx_media_upload_ctx_t *upload_ctx, ngx_str_t *content_disposition) { /* {{{ */
...
static ngx_int_t upload_parse_part_header(ngx_media_upload_ctx_t *upload_ctx, char *header, char *header_end) { /* {{{ */
    ngx_str_t s;
...
static void upload_discard_part_attributes(ngx_media_upload_ctx_t *upload_ctx) { /* {{{ */
    ...
    upload_ctx->output_name.len = 0;
    upload_ctx->output_name.data = NULL;

    upload_ctx->output_prefix.len = 0;
    upload_ctx->output_prefix.data = NULL;
...
static ngx_int_t upload_start_file(ngx_media_upload_ctx_t *upload_ctx) { /* {{{ */
...
static void upload_finish_file(ngx_media_upload_ctx_t *upload_ctx) { /* {{{ */
...
static void upload_abort_file(ngx_media_upload_ctx_t *upload_ctx) { /* {{{ */
...
static void upload_flush_output_buffer(ngx_media_upload_ctx_t *upload_ctx) { /* {{{ */
...
static void upload_init_ctx(ngx_media_upload_ctx_t *upload_ctx) { /* {{{ */
	...
	upload_ctx->start_part_f = ngx_media_upload_start_handler;
	upload_ctx->finish_part_f = ngx_media_upload_finish_handler;
	upload_ctx->abort_part_f = ngx_media_upload_abort_handler;
	upload_ctx->flush_output_buffer_f = ngx_media_upload_flush_output_buffer;
...
static ngx_int_t /* {{{ ngx_media_upload_validate_session_id */
ngx_media_upload_validate_session_id(ngx_str_t *session_id) {
    u_char *p, *q;
...
static ngx_int_t upload_parse_request_headers(ngx_media_upload_ctx_t *upload_ctx, ngx_http_headers_in_t *headers_in) { /* {{{ */
    ...
    ulcf = ngx_http_get_module_loc_conf(upload_ctx->request, ngx_media_upload_module);
    ...
    if(ngx_strncasecmp(content_type->data, (u_char*) MULTIPART_FORM_DATA_STRING,
        ...
        for (i = 0;;i++) {
                ...
                || !strncasecmp(X_SESSION_ID_STRING, (char*)header[i].key.data, sizeof(X_SESSION_ID_STRING) - 1 - 1))
            {
                ...
                if(ngx_media_upload_validate_session_id(&header[i].value) != NGX_OK) {
                    ngx_log_debug0(NGX_LOG_DEBUG_CORE, upload_ctx->log, 0,
                ...
                || !strncasecmp(X_CONTENT_RANGE_STRING, (char*)header[i].key.data, sizeof(X_CONTENT_RANGE_STRING) - 1 - 1))
            {
                ...
                if(ngx_media_upload_parse_range(&s, &upload_ctx->content_range_n) != NGX_OK) {
...
static ngx_int_t /* {{{ ngx_media_upload_parse_range */
ngx_media_upload_parse_range(ngx_str_t *range, ngx_media_upload_range_t *range_n)
{
    u_char *p = range->data;
...
static void upload_putc(ngx_media_upload_ctx_t *upload_ctx, u_char c) { /* {{{ */
...
static ngx_int_t
upload_process_raw_buf(ngx_media_upload_ctx_t *upload_ctx, u_char *start, u_char *end) { /* {{{ */
    ngx_int_t rc;
...
static ngx_int_t /* {{{ */
ngx_media_upload_test_expect(ngx_http_request_t *r)
{
    ngx_int_t   n;
    ngx_str_t  *expect;
...
static void
ngx_media_upload_get_suffix(ngx_pool_t* pool,ngx_str_t* file_name,ngx_str_t* suffix)
{
    ngx_int_t index = file_name->len -1;
    u_char data     = 0;
    ngx_int_t find  = 0;
    ...
    size_t strLen = file_name->len - index;
    size_t size = strLen;

    suffix->data = ngx_pcalloc(pool,size);
    suffix->len  = size;
    (void)ngx_copy(suffix->data, &file_name->data[index], strLen);
